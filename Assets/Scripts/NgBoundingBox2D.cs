...
namespace ProjectNothing
{
    public struct NgBoundingBox2D
    {
        ...
        public NgBoundingBox2D (Vector2 center, Vector2 size)
        {
            m_Center = center;
            m_Extents = size * 0.5f;
        }
        ...
        public void Encapsulate (NgBoundingBox2D bound)
        {
        ...
        public readonly bool Contains (NgBoundingBox2D bound) => Contains (bound.Min) && Contains (bound.Max);

        public readonly bool Contains (Vector2 point) => Min.x <= point.x && Max.x >= point.x && Min.y <= point.y && Max.y >= point.y;

        public readonly bool Intersects (NgBoundingBox2D bound) => Min.x <= bound.Max.x && Max.x >= bound.Min.x && Min.y <= bound.Max.y && Max.y >= bound.Min.y;
    }
}
